...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    if (storedPlan && ["basic", "pro", "ultra"].includes(storedPlan)) {
      ...
      ["basic", "pro", "ultra"].includes(subscription.selected_plan)
    ) {
  ...
  useEffect(() => {
    const processPlanChange = async () => {
      ...
      if (!hasActiveSubscription || subscription?.active_plan === "basic")
        return;
      ...
      try {
        ...
        if (data?.error) {
          ...
          toast({
            title: "Plan Updated",
            description: `Your subscription has been changed to ${planParam.toUpperCase()}`,
          });
          navigate("/dashboard", { replace: true });
        }
      } catch (err: any) {
    ...
    if (selectedPlan === "basic") {
      try {
        ...
        toast({
          title: "Error",
          description:
            err.message || "Failed to activate Basic plan. Please try again.",
          variant: "destructive",
        });
        return;
      }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Basic is free forever, or try Pro or Ultra free for 7 days. Cancel
            anytime.
          </p>
        </div>
          ...
          >
            {isRedirecting ? (
              ...
              <>
                ...
                Start 7-Day Free Trial — ${pricing[selectedPlan][billingPeriod]}
                /{isYearly ? "year" : "month"} after
              </>
            )}
          </Button>
        </div>
