...
function createEmptyField(type: FieldType = 'text'): Field {
  let label = 'New Field';
  switch (type) {
    case 'text': label = 'Full Name'; break;
    case 'number': label = 'Age'; break;
    case 'email': label = 'Email Address'; break;
    case 'phone': label = 'Phone Number'; break;
    case 'date': label = 'Date of Birth'; break;
    case 'dropdown': label = 'Country'; break;
    case 'checkbox': label = 'Agree to Terms'; break;
    case 'radio': label = 'Gender'; break;
    case 'textarea': label = 'Additional Comments'; break;
    // Add cases for other FieldType values if specific default labels are needed
    default: label = type.charAt(0).toUpperCase() + type.slice(1); break;
  }
  return {
    id: uuidv4(),
    type,
...
const TemplateBuilder = () => {
  ...
  const handleAddColumn = (sectionId: string) => {
    setSections(prev => prev.map(s => {
      if (s.id === sectionId && s.columns.length < 2) { // Max 2 columns for now
        return { ...s, columns: [...s.columns, { id: uuidv4() + '-col', fields: [] }] };
      }
      return s;
    }));
  };

  const handleAddField = (sectionId: string, columnId: string, type: FieldType = 'text') => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    if (draggedSidebarType && event.over && typeof event.over.id === 'string' && event.over.id.startsWith('column-dropzone-')) {
  ...
  const handleSaveTemplate = async (status: 'draft' | 'published' = 'draft') => {
    ...
    if (error) {
      toast({ title: `Error ${status === 'draft' ? 'saving' : 'publishing'} template`, description: error.message, variant: 'destructive' });
    } else {
      toast({ title: `Template ${status === 'draft' ? 'saved as draft' : 'published'}` });
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-3">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <Button variant="outline" size="icon" onClick={() => navigate('/admin/templates')} className="h-9 w-9">
            ...
            {editingTitle ? (
              ...
              <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setEditingTitle(true)}>
                <h1 className="text-xl font-semibold text-gray-800 group-hover:text-primary-600">{templateName}</h1>
                <Edit3 size={16} className="text-gray-400 group-hover:text-primary-500 transition-opacity opacity-0 group-hover:opacity-100" />
              </div>
            )}
          </div>
          <div className="flex gap-2">
            ...
            <Button 
              ...
              className="text-gray-700 hover:text-primary-600"
            >
              {isSaving ? <span className="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full mr-2"></span> : <Save className="h-4 w-4 mr-2" />}
              Save Draft
            </Button>
            <Button onClick={() => handleSaveTemplate('published')} disabled={isSaving || isPublishing} className="bg-primary-600 hover:bg-primary-700 text-white">
              {isPublishing ? <span className="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full mr-2"></span> : <Upload className="h-4 w-4 mr-2" />}
              Publish
            </Button>
          </div>
        </div>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <main className="flex-1 flex gap-0 overflow-hidden"> {/* Changed gap-4 to gap-0 for tighter layout */}
          {activeView === 'edit' ? (
            <>
              <aside className="w-72 bg-white border-r border-gray-200 overflow-y-auto scrollbar-thin"> {/* Width 72, was 64 */}
                <ToolbarPanel draggable /> {/* Removed onDragStartSidebar */}
              </aside>
              
              <section className="flex-1 bg-gray-100 p-6 md:p-8 overflow-y-auto scrollbar-thin"> {/* Added padding */}
                <TemplateCanvas
                  elements={sections}
          ...
          <DragOverlay>{draggedSidebarType && <div className="p-2 bg-primary-500 text-white rounded shadow-lg font-medium cursor-grabbing">{draggedSidebarType.charAt(0).toUpperCase() + draggedSidebarType.slice(1)} Field</div>}</DragOverlay>
        </main>
      </DndContext>
    </div>
  );
