  ...
  return <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-4">
        <div className="h-[400px] overflow-y-auto border rounded-md p-4 bg-background">
          {messages.map((message, index) => <div key={index} className={`mb-4 p-3 rounded-lg ${message.role === 'assistant' ? 'bg-primary/10 mr-12' : 'bg-secondary ml-12'}`}>
        ...
        <form onSubmit={handleSubmit} className="flex gap-2 items-end">
          <div className="flex-1">
            <Textarea placeholder="Describe the document you need... (e.g., 'I need a template for a rental agreement for commercial property in Delhi')" value={prompt} onChange={e => setPrompt(e.target.value)} className="min-h-[80px]" />
          </div>
          <Button type="submit" className="gradient-blue" disabled={isLoading || !prompt.trim()}>
