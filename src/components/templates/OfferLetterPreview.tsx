...
export const OfferLetterPreview: React.FC<ExtendedOfferLetterPreviewProps> = ({
  data,
  ...
  useEffect(() => {
    const fetchBranding = async () => {
      if (!institutionName && !user?.id) {
        console.warn(
          "Offer Letter Preview: Institution name or user context not available for fetching branding.",
        );
        return;
      }
      try {
  ...
  return (
    <div className="a4-document p-8 bg-white text-gray-800 font-sans text-sm leading-relaxed">
      ...
      <div className="flex justify-between mb-6 text-sm">
        <p>
          <strong>Reference No.:</strong> OL/{new Date().getFullYear()}/______
        </p>
        <p>
          <strong>Date:</strong> {formatDate(dateOfOffer)}
        </p>
      </div>
      ...
      <div className="mb-6">
        <p>To,</p>
        <p>
          <strong>{candidateName || "[Candidate Name]"}</strong>
        </p>
      ...
      <div className="mb-6">
        <p>
          <strong>
            Subject: Offer of Employment - {jobTitle || "[Job Title]"}
          </strong>
        </p>
      </div>
      ...
      <div className="space-y-4 text-base leading-7">
        ...
        <p className="text-justify">
          ...
          <strong>{jobTitle || "[Job Title]"}</strong> in the
          <strong> {department || "[Department]"}</strong> department at{" "}
          <strong>{institutionName || "[Institution Name]"}</strong>. You will
          be reporting to{" "}
          <strong>{reportingManager || "[Reporting Manager]"}</strong>.
        </p>

        <p className="text-justify">
          Your anticipated start date will be{" "}
          <strong>{formatDate(startDate)}</strong>. This offer is subject to
          successful completion of pre-employment verification and a probation
          period of <strong>{probationPeriod || "[Probation Period]"}</strong>.
        </p>
        ...
        <div className="space-y-2">
          ...
          <div className="ml-4 space-y-1">
            <p>
              • {salaryFrequency === "annually" ? "Annual" : "Monthly"} Salary:{" "}
              <strong>{formatCurrency(salaryAmount, salaryCurrency)}</strong>
            </p>
            <p>
              • Work Hours: <strong>{workHours || "[Work Hours]"}</strong>
            </p>
            <p>
              • Work Location:{" "}
              <strong>{workLocation || "[Work Location]"}</strong>
            </p>
          </div>
        </div>
        ...
        <p className="text-justify">
          ...
          <strong>{formatDate(acceptanceDeadline)}</strong>. Please confirm your
          acceptance by signing and returning a copy of this letter by the
          specified date.
        </p>

        <p className="text-justify">
          We look forward to welcoming you to our team and are confident that
          your skills and experience will be valuable additions to our
          organization.
        </p>
