...
export const AddressProofPreview: React.FC<
  ExtendedAddressProofPreviewProps
  ...
  useEffect(() => {
    ...
    if (fullName && currentAddress && purpose) {
      const generateQR = async () => {
        ...
        if (!url) {
          console.error(
            "QR code URL generation failed for Address Proof Preview.",
          );
        }
      };
      generateQR();
    ...
    <div className="a4-document p-8 bg-white text-gray-800 font-sans text-sm leading-relaxed relative">
      ...
      <div className="space-y-4 text-justify leading-7 text-base">
        <p>
          ...
          is currently residing at the following address:
        </p>
        ...
        <p>
          The above mentioned person has been residing at the current address
          for the past <strong>{residenceDuration || "[Duration]"}</strong>.
        </p>

        <p>
          This certificate is issued for{" "}
          <strong>{purpose || "[Purpose]"}</strong> purposes at the request of
          the concerned person.
        </p>

        <p>
          The information provided above is true and accurate to the best of our
          knowledge.
        </p>
      </div>
