...
interface ExtendedMaternityLeavePreviewProps
  extends MaternityLeavePreviewProps {
  isEmployeePreview?: boolean;
  requestStatus?: "pending" | "approved" | "rejected";
}

export const MaternityLeavePreview: React.FC<
  ExtendedMaternityLeavePreviewProps
  ...
  useEffect(() => {
    const generateQR = async () => {
      if (fullName && leaveStartDate && leaveEndDate) {
        ...
        if (!url) {
          console.error(
            "QR code URL generation failed for Maternity Leave Preview.",
          );
        }
      }
    };
    ...
    <div className="a4-document p-8 bg-white text-gray-800 font-sans text-sm leading-relaxed relative">
      ...
      <div className="space-y-4 text-justify leading-7 text-base">
        <p>
          ...
          organization, has been granted maternity leave.
        </p>
          ...
          <table className="w-full border-collapse border border-gray-400">
            <tbody>
              ...
              <tr>
                ...
                <td className="border border-gray-400 p-2">
                  {totalLeaveDays || "[Total Days]"} days
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <p>
          The employee is expected to resume duties on{" "}
          {formatDate(leaveEndDate)} or as per medical advice.
        </p>

        <p>This certificate is issued for official purposes and records.</p>
      </div>
