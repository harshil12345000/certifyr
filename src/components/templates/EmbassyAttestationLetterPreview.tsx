  ...
  useEffect(() => {
    const fetchBranding = async () => {
      if (!institutionName && !user?.id) {
        console.warn("Embassy Attestation Letter Preview: Institution name or user context not available for fetching branding.");
        return;
      }
      try {
  ...
  return (
    <div className="a4-document p-8 bg-white text-gray-800 font-serif text-sm leading-relaxed">
      ...
      <div className="space-y-4 text-justify leading-7">
        ...
        <p>
          This is to certify and confirm the authenticity of the document(s) submitted by <strong>{applicantName || '[Applicant Name]'}</strong> for the purpose of embassy attestation.
        </p>

        <div className="my-6">
          <h3 className="font-semibold mb-3 text-blue-600">Applicant Details:</h3>
          <div className="ml-4 space-y-1 bg-blue-50 p-4 rounded">
            <p><strong>Full Name:</strong> {applicantName || '[Applicant Name]'}</p>
        ...
        <div className="my-6">
          <h3 className="font-semibold mb-3 text-blue-600">Document Details:</h3>
