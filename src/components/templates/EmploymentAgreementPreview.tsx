  ...
  return (
    <div className="a4-document bg-white p-8 shadow-lg max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p><strong>Start Date:</strong> {data.startDate ? new Date(data.startDate).toLocaleDateString() : '[Date]'}</p>
        <p><strong>Work Location:</strong> {data.workLocation}</p>
        <p><strong>Work Hours:</strong> {data.workHours}</p>
        {data.probationPeriod && <p><strong>Probation Period:</strong> {data.probationPeriod}</p>}
      </div>

      <div className="mb-6">
        <h3 className="text-lg font-bold mb-2">COMPENSATION</h3>
        <p><strong>Salary:</strong> {data.salary}</p>
        <p><strong>Pay Frequency:</strong> {data.payFrequency}</p>
        {data.benefits && (
          <div className="mt-2">
      ...
      <div className="mt-12">
        <p className="mb-6">By signing below, both parties acknowledge that they have read, understood, and agree to be bound by the terms of this Employment Agreement.</p>
