  ...
  useEffect(() => {
    const fetchBranding = async () => {
      if (!institutionName && !user?.id) {
        console.warn("Character Preview: Institution name or user context not available for fetching branding.");
        return;
      }
      try {
        ...
        if (!orgIdToQuery) {
          console.warn("Character Preview: Could not determine organization ID for branding assets.");
        }
    ...
    <div className="a4-document p-8 bg-white text-gray-800 font-serif text-sm leading-relaxed" style={documentStyle}>
      ...
      <div className="text-center mb-8">
        <div className="border border-gray-400 inline-block px-8 py-3">
          <h2 className="font-bold uppercase tracking-widest" style={{ fontSize: `${fontSize * 1.5}px` }}>CHARACTER CERTIFICATE</h2>
        </div>
      </div>
      ...
      <div className="space-y-4 text-justify" style={{ lineHeight: `${fontSize * 1.8}px` }}>
        <p>
          This is to certify that <strong>{fullName || '[Full Name]'}</strong>, son/daughter of <strong>{parentName || "[Parent's Name]"}</strong>, residing at <strong>[{address || 'Address'}]</strong>, is known to me for the past <strong>[{duration || 'Duration'}]</strong>.
        </p>

        <p>
          During this period, I have found [him/her] to be of <strong>[{conduct || 'Character & Conduct'}]</strong>. [He/She] bears a good moral character and has never been involved in any criminal or anti-social activities.
        </p>

        <p>
          I have no hesitation in recommending [him/her] for any suitable employment or educational opportunities.
        </p>

        <p>
          This certificate is issued upon the request of the individual for official purposes.
        </p>
      </div>
