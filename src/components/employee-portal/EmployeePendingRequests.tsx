  ...
  if (requests.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground mb-4">You haven't submitted any document requests yet.</p>
        <Button onClick={() => window.location.href = `/${organizationId}/request-portal`}>
          Browse Templates
        </Button>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {requests.map((request) => {
          const PreviewComponent = Previews[TEMPLATE_PREVIEW_MAP[request.template_id]];
            ...
            <Card key={request.id}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg">{getTemplateName(request.template_id)}</CardTitle>
                    <CardDescription>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                        <DialogHeader>
                          <DialogTitle>{getTemplateName(request.template_id)}</DialogTitle>
                          <DialogDescription>
              ...
              {request.processed_at && (
                <CardContent className="pt-0">
                  <p className="text-sm text-muted-foreground">
                    {request.status === 'approved' ? 'Approved' : 'Processed'} on {new Date(request.processed_at).toLocaleDateString()}
                  </p>
                </CardContent>
              )}
            </Card>
