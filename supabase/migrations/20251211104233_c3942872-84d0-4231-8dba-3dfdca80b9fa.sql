-- Remove redundant profiles table
-- The profiles table only stores (id, email, full_name, created_at, updated_at) 
-- while user_profiles stores comprehensive user data
-- Nowhere in the codebase queries 'profiles' table - only user_profiles is used

-- Step 1: Update the handle_new_user trigger to stop inserting into profiles table
...
-- Step 2: Drop the redundant profiles table (no data needs migration since user_profiles has all needed data)
